...
class Generate extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->config = [
            'theme' => 'docgen',
            'title' => 'Generated by DocGen',
        ];

        if (file_exists('docgen.json')) {
            $json = json_decode(file_get_contents('docgen.json'), true);
        ...
        if (!empty($this->config['theme'])) {
            if ($this->config['theme'] == 'docgen' || !file_exists(dirname($this->getThemePath('file', $this->config['theme'])))) {
                $this->config['theme'] = false;
            }
        }

        $output->writeln('<comment>Generating PDF</comment>');
        ...
        if ($coverPageHtml) {
            file_put_contents('docgen_cover.html', $coverPageHtml);
            $pdf->addCover('docgen_cover.html');
            $this->config['hasCover'] = true;
        }

        $pdf->setOptions([
            'header-html' => 'docgen_header.html',
            'footer-html' => 'docgen_footer.html',
        ]);
        ...
        file_put_contents('docgen_header.html', $headerHtml);
        file_put_contents('docgen_body.html', $bodyHtml);
        file_put_contents('docgen_footer.html', $footerHtml);

        $pdf->addToc([
            'xsl-style-sheet' => $this->getToc()
        ]);

        $pdf->addPage('docgen_body.html');
    ...
    private function getThemeFile($file, $variables = [], $baseTheme = false)
    {
        ...
        $filename = $this->getThemePath($file, !$baseTheme ? $this->config['theme'] : 'docgen');
    ...
    {
        $baseToc = $this->getThemePath('toc.xsl', 'docgen');
        $themeToc = $this->getThemePath('toc.xsl', !empty($this->config['theme']) ? $this->config['theme'] : 'docgen');
